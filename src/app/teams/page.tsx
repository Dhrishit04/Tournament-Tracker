...
import { FadeUp, StaggeredGrid, TiltCard, gridItemVariants } from '@/components/ui/animations';
...
export default function TeamsPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-24 max-w-6xl relative z-10">
      <div className="mb-12 text-center md:text-left">
        <FadeUp delay={0.1}>
          <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-3 flex flex-wrap gap-[0.2em] justify-center md:justify-start">
            Club <span className="text-gradient-purple">Directory</span>
          </h1>
        </FadeUp>
        <motion.p
        ...
        >
          Explore team identities, rosters, and season statistics.
        </motion.p>
        <motion.div
      ...
      {teams.length > 0 ? (
        <StaggeredGrid staggerDelay={0.08} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {teams.map((team) => {
            const logo = getImageUrl(team.logoUrl);
            return (
              <motion.div key={team.id} variants={gridItemVariants} className="h-full">
                <TiltCard className="h-full">
                  <Card className="flex flex-col h-full overflow-hidden bento-card border-none bg-transparent">
