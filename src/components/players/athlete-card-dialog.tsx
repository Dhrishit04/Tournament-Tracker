            ...
            <div className="relative p-6 md:p-8 space-y-8">
              <div className="flex flex-col items-center text-center space-y-4">
                <motion.div 
                    ...
                    className={cn(
                    "relative w-32 h-32 rounded-3xl overflow-hidden border-4 shadow-2xl transition-all duration-700",
                    isClassA ? "border-accent shadow-accent/30" : "border-white/10"
                    )}
                >
                  <Image src={avatar.imageUrl} alt={player.name} fill className="object-cover group-hover:scale-110 transition-transform duration-500" data-ai-hint={avatar.imageHint} />
                </motion.div>
                ...
                >
                  ...
                  <h2 className="text-3xl font-black italic tracking-tighter uppercase leading-none mt-2">{player.name}</h2>
                  {team && (
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center justify-center gap-2 mt-1">
              ...
              >
                <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex flex-col items-center justify-center text-center shadow-inner">
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-2 border-b border-white/5 pb-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Performance Analytics</span>
                </div>
              ...
              <motion.div 
                ...
                className="pt-4 pb-2 flex items-center justify-between border-t border-white/5"
              >
                ...
                <div className="flex gap-2">
                  {player.yellowCards > 0 && (
                    <div className="w-4 h-6 bg-yellow-400/80 rounded-sm border border-black/20 animate-in fade-in zoom-in" title={`${player.yellowCards} Yellow Cards`} />
                  )}
                  {player.redCards > 0 && (
                    <div className="w-4 h-6 bg-red-600/80 rounded-sm border border-black/20 animate-in fade-in zoom-in" title={`${player.redCards} Red Cards`} />
                  )}
                </div>
              </motion.div>
            </div>
