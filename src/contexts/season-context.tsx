...
import type { Season, MatchConfig, GlobalAnnouncement } from '@/types';
...
export const SeasonProvider = ({ children }: { children: ReactNode }) => {
    ...
    const deleteCurrentSeason = useCallback(async () => {
        ...
        const seasonToDelete = currentSeason;
        const remainingSeasons = seasons.filter(s => s.id !== seasonToDelete.id);
        const newCurrentSeason = [...remainingSeasons].sort((a,b) => b.year - a.year)[0];
    
        const configRef = doc(firestore, 'config', 'app');
        updateDoc(configRef, {
            seasons: remainingSeasons,
            currentSeasonId: newCurrentSeason.id
        }).then(() => {
            logAction("DELETE_SEASON", `Permanently decommissioned ${seasonToDelete.name}`);
            toast({ title: 'Season Deleted', description: `${seasonToDelete.name} has been removed.` });
        }).catch(async () => {
            errorEmitter.emit('permission-error', new FirestorePermissionError({
                path: configRef.path,
                operation: 'update'
    ...
    const updateMatchConfig = useCallback(async (newConfig: MatchConfig) => {
        ...
        const logs: string[] = [];
        if (old.isGroupModeActive !== newConfig.isGroupModeActive) {
            logs.push(newConfig.isGroupModeActive ? "ENABLED Group Mode" : "DISABLED Group Mode");
        }
        if (old.showGroupStage !== newConfig.showGroupStage) logs.push(`Group Stage: ${newConfig.showGroupStage ? 'Visible' : 'Hidden'}`);
        if (old.showQuarterFinals !== newConfig.showQuarterFinals) logs.push(`Quarters: ${newConfig.showQuarterFinals ? 'Visible' : 'Hidden'}`);
        if (old.showOthers !== newConfig.showOthers) logs.push(`Others Stage: ${newConfig.showOthers ? 'Visible' : 'Hidden'}`);
        if (old.showVenue !== newConfig.showVenue) logs.push(`Venue Info: ${newConfig.showVenue ? 'Visible' : 'Hidden'}`);
        ...
        const updatedSeasons = seasons.map(s => 
            s.id === currentSeason.id ? { ...s, matchConfig: newConfig } : s
        );
